...
export class DealPinnedVisibilityWatcherService {
    ...
    private async handleBatch(): Promise<void> {
        const now = new Date();
        const minPostAgeMs =
            PIN_VISIBILITY_CONFIG.MIN_POST_AGE_MINUTES * 60 * 1000;
    ...
    private async checkPublication(
        ...
        try {
            pinnedIds = await this.telegramChannelPinsService.getPinnedMessageIds(
                chatId,
            );
        } catch (error) {
            await this.handlePinAccessError(publication.id, error, now);
        ...
        const outcome = await this.dataSource.transaction(async (manager) => {
            const publicationRepo =
                manager.getRepository(DealPublicationEntity);
            ...
            const missingFirstSeenAt =
                lockedPublication.pinMissingFirstSeenAt ?? now;
            const graceLimit =
                PIN_VISIBILITY_CONFIG.MISSING_GRACE_CHECKS + 1;
            ...
            if (missingCount >= graceLimit) {
                await publicationRepo.update(lockedPublication.id, {
                    ...
                    pinMissingLastCheckedAt: now,
                    pinMissingFinalizedAt:
                        lockedPublication.pinMissingFinalizedAt ?? now,
                    pinLastErrorCode: null,
                });
