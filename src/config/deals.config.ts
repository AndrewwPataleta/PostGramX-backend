...
const stageLikeEnvs: string[] = [
  NODE_ENV_VALUES.STAGE,
  NODE_ENV_VALUES.PRODUCTION,
];
const isStageOrProduction = stageLikeEnvs.includes(
...
const parseBoolean = (
  ...
  return value === ENV_LITERAL_VALUES.TRUE;
};
...
export const DEALS_CONFIG = {
  ...
  MOCK_CREATIVE_APPROVE:
    process.env.DEALS_MOCK_CREATIVE_APPROVE === ENV_LITERAL_VALUES.TRUE,
...
if (
  process.env[ENV.NODE_ENV] === NODE_ENV_VALUES.PRODUCTION &&
